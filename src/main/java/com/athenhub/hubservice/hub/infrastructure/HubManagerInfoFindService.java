...
/**
 * 허브 담당자(HubManager) 정보를 조회하는 서비스 구현체.
 *
 * <p>해당 서비스는 외부 회원 서비스(member-service)와 연동하여 허브 담당자 정보를 조회하고, 조회된 회원 정보를 기반으로 {@link HubManager}
 * 도메인 모델로 변환하여 반환한다.
 *
 * <h2>역할</h2>
 *
 ...
 * </ul>
 *
 * <p>구현체는 {@link HubManagerInfoFinder} 인터페이스를 통해 사용되며, 허브 담당자 관련 기능에서 조회 책임을 담당한다.
 *
...
public class HubManagerInfoFindService implements HubManagerInfoFinder {
  ...
  @Override
  public HubManagerInfo find(HubManagerId agentId) {
    return memberServiceClient.getMemberInfo(agentId.toUuid()).toHubManagerInfo();
  }
}
